...
import { insertOrderSchema, insertFreeTrialSchema, insertFeedbackSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/free-trial", async (req, res) => {
    try {
      ...
      const { sendFreeTrialConfirmation, sendShareableLink } = await import("./whatsapp");
      ...
      if (!confirmationSent) {
        console.warn("WhatsApp confirmation message failed for trial:", trial.id);
      }
      ...
      if (confirmationSent) {
        ...
        if (!shareableLinkSent) {
          console.warn("WhatsApp shareable link message failed for trial:", trial.id);
        }
      }
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      const { sendTemplateMessage, normalizePhoneNumber } = await import("./whatsapp");
      const normalizedPhone = normalizePhoneNumber(validatedData.customerPhone);
      const whatsappSent = await sendTemplateMessage(normalizedPhone, "hello_world", "en_US");
      
      if (!whatsappSent) {
        console.warn("WhatsApp confirmation message failed for order:", order.id);
      }
  ...
  app.get("/api/feedback/check/:orderCode", async (req, res) => {
    try {
      const feedback = await storage.getFeedbackByOrderCode(req.params.orderCode);
      res.json({
        submitted: !!feedback,
  ...
  app.post("/api/feedback/submit", async (req, res) => {
    try {
      ...
      const existingFeedback = await storage.getFeedbackByOrderCode(validatedData.orderCode);
      if (existingFeedback) {
  ...
  app.post("/webhooks/payment", async (req, res) => {
    try {
      ...
      if (webhookSecret) {
        ...
        console.warn('⚠️  DEVELOPMENT MODE: RAZORPAY_WEBHOOK_SECRET not set - skipping signature verification');
        console.warn('⚠️  DO NOT USE IN PRODUCTION WITHOUT SIGNATURE VERIFICATION');
      }
      ...
      const internalStatus = event === 'payment.captured' ? 'payment_succeeded' : 
                           event === 'payment.failed' ? 'payment_failed' : 
                           status;
      ...
      if (amount !== undefined && amount !== null) {
        if (typeof amount !== 'number' || !Number.isFinite(amount)) {
          console.error('Invalid amount type:', { orderId: order_id, amount, type: typeof amount });
      ...
      const { normalizePhoneNumber, validateE164, sendTemplateMessageWithRetry, sendTextMessageWithRetry } = await import("./whatsapp");
      ...
      const whatsappBusinessNumber = process.env.WHATSAPP_BUSINESS_NUMBER_E164 || customerPhoneE164;
      const appBaseUrl = process.env.APP_BASE_URL || 'https://your-domain.replit.app';
      ...
      const linkMessageSent = await sendTextMessageWithRetry(customerPhoneE164, linkMessage);
  ...
  app.get("/w/invite/:token", async (req, res) => {
    try {
      ...
      const whatsappBusinessNumber = process.env.WHATSAPP_BUSINESS_NUMBER_E164 || '919876543210';
      const waLink = `https://wa.me/${whatsappBusinessNumber}?text=${encodeURIComponent(
        `Hi, I'm contacting on behalf of order ${order.uniqueCode}. Token: ${token}`
      )}`;
