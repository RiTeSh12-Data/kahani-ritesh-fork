...
import { sendTextMessageWithRetry, sendStorytellerOnboarding, sendReadinessCheck, sendVoiceNoteAcknowledgment, sendAlbumCompletionMessage, downloadVoiceNoteMedia, downloadMediaFile } from "./whatsapp";
import { getQuestionByIndex, getTotalQuestionsForAlbum } from "@shared/albumQuestions";
...
async function handleInitialContact(trial: any, fromNumber: string): Promise<void> {
...
async function handleReadinessResponse(trial: any, fromNumber: string, response: string): Promise<void> {
  ...
  const isMaybe = maybePatterns.some(pattern => normalizedResponse.includes(pattern));
...
async function sendFirstQuestion(trial: any, fromNumber: string): Promise<void> {
...
async function handleVoiceNote(trial: any, fromNumber: string, message: WhatsAppMessage): Promise<void> {
  ...
  const currentQuestion = getQuestionByIndex(trial.selectedAlbum, trial.currentQuestionIndex);
  
  if (!currentQuestion) {
    console.error('No current question found for question index:', trial.currentQuestionIndex);
    return;
  }
  ...
  if (alreadyAnswered) {
    console.log('Voice note already exists for this question, skipping duplicate insert');
  } else {
    try {
      ...
      downloadAndStoreVoiceNote(voiceNote.id, audioId, mimeType || "audio/ogg").catch(console.error);
    } catch (error) {
...
async function downloadAndStoreVoiceNote(voiceNoteId: string, mediaId: string, mimeType: string): Promise<void> {
  try {
    ...
    if (!fileBuffer) {
      console.error('Failed to download media file for voice note:', voiceNoteId);
      await storage.updateVoiceNote(voiceNoteId, {
        downloadStatus: 'failed',
      });
      return;
    ...
    if (!supabaseUrl) {
      console.error('Failed to upload voice note to Supabase Storage:', voiceNoteId);
    ...
    const fileExtension = finalMimeType.includes('ogg') ? 'ogg' : 
                         finalMimeType.includes('mp3') ? 'mp3' : 
                         finalMimeType.includes('m4a') ? 'm4a' : 'ogg';
